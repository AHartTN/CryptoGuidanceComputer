// Apollo DSKY - Base Blockchain Provider Abstract Class
...
import { IBlockchainData, ITransactionRequest, ITokenBalance } from '../interfaces/IWeb3Operations';
...
export abstract class BaseBlockchainProvider implements IBlockchainProvider {
  ...
  async getTokenBalances(address: string, tokenContracts: string[]): Promise<ITokenBalance[]> {
    ...
    for (const contract of tokenContracts) {
      try {
        ...
        console.warn(`Failed to get balance for token ${contract}:`, error);
      }
    }
  ...
  async waitForTransaction(hash: string): Promise<any> {
    ...
    throw this.createError(BlockchainErrorType.TransactionFailed, `Transaction ${hash} timeout`);
  }
  ...
  protected createError(type: BlockchainErrorType, message: string, code?: number, data?: any): IBlockchainError {
    return {
      type,
      message,
      code,
