...
import type { IPriceAlert as IUnifiedPriceAlert } from "../../interfaces/IPriceAlert";
...
export class RealTimePriceFeedService {
  ...
  private alerts: Map<string, IUnifiedPriceAlert> = new Map();
  private subscriptions: Set<string> = new Set();
  private priceHistory: Map<string, IPriceHistoryPoint[]> = new Map();

  // Event callbacks
  private onPriceUpdate?: (data: ICryptoPriceData) => void;
  private onAlert?: (alert: IUnifiedPriceAlert, data: ICryptoPriceData) => void;
  ...
  async subscribeToSymbol(symbol: string): Promise<string> {
    ...
    const subscriptionId = this.wsService.subscribe({
      ...
      errorCallback: (error) =>
        console.error(`[PriceFeed] Error for ${symbol}:`, error),
    });
  ...
  getCurrentPrice(symbol: string): ICryptoPriceData | null {
    return this.cache.get(`price:${symbol.toUpperCase()}`) as ICryptoPriceData | null;
  }
  ...
  /** Add price alert */
  addAlert(alert: IUnifiedPriceAlert): void {
    this.alerts.set(alert.id, alert);
    console.log(
      "[PriceFeed] Added alert for",
      alert.symbol,
      alert.condition,
      alert.threshold,
    );
  ...
  getStats() {
    return {
      ...
      historyPoints: Array.from(this.priceHistory.values()).reduce(
  ...
  setEventCallbacks(callbacks: {
    onPriceUpdate?: (data: ICryptoPriceData) => void;
    onAlert?: (alert: IUnifiedPriceAlert, data: ICryptoPriceData) => void;
  ...
  private handlePriceUpdate(data: unknown): void {
    ...
    try {
      ...
      if (
        ...
        this.priceHistory.has(priceData.symbol)
      ) {
        const history = this.priceHistory.get(priceData.symbol)!;
        history.push({
          timestamp: priceData.lastUpdated as number,
  ...
  private checkAlerts(data: ICryptoPriceData): void {
    this.alerts.forEach((alert) => {
      ...
      switch (alert.condition) {
        ...
        case "below":
          ...
          break;
        case "change_percent":
          triggered = Math.abs(data.changePercent24h ?? 0) > alert.threshold;
          break;
      }
