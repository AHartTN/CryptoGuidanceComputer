...
export class SecurityService {
  ...
  private onSuspiciousActivity?: (source: string, events: ISecurityEvent[]) => void;

  constructor(private config: ISecurityConfig) {    this.cache = new CacheService({
  ...
  validateInput(input: unknown, rules: IValidationRule[]): IValidateInputResult {
    if (typeof input !== 'object' || input === null) {
      return { isValid: false, sanitizedInput: {}, errors: ['Input must be an object'], securityEvents: [] };
    }
    ...
    if (!this.config.enableInputValidation) {
      return { isValid: true, sanitizedInput: input as Record<string, unknown>, errors: [], securityEvents: [] };
    }
    ...
    for (const rule of rules) {
      ...
      try {
        ...
        if (rule.pattern && typeof value === 'string' && !rule.pattern.test(value)) {
          ...
          securityEvents.push(this.createSecurityEvent(
        ...
        errors.push(`Validation error for ${rule.field}: ${(error as Error).message}`);
        isValid = false;
      }
    }
  ...
  checkRateLimit(source: string, customConfig?: Partial<IRateLimitConfig>): ICheckRateLimitResult {
  ...
  validateTransactionHash(hash: string): IValidateTransactionHashResult {
    ...
    if (cleanHash.length !== 64) {
      errors.push('Transaction hash must be 64 characters long (excluding 0x prefix)');
      return { isValid: false, errors };
    }
  ...
  getSecurityStats(): {
    ...
    this.securityEvents.forEach(event => {
      ...
      if (event.type === SecurityEventType.RATE_LIMIT_EXCEEDED) rateLimitViolations++;
    });
  ...
  /** Validate type */
  private validateType(value: unknown, rule: IValidationRule): {
    isValid: boolean;
    errors: string[];
    securityEvents: ISecurityEvent[];
  } {
    ...
    switch (rule.type) {
      ...
      case 'email':
        if (typeof value !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value as string)) {
  ...
  private performSecurityChecks(value: unknown, field: string): {
    ...
    if (this.config.enableInjectionProtection && this.detectSQLInjection(value)) {
      events.push(this.createSecurityEvent(
