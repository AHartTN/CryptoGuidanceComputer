/**
 * Executes DSKY verb/noun commands, dispatching to blockchain and crypto services.
 *
 * @file DSKYCommandExecutor.ts
 */

import {
  DSKYVerb,
  DSKYNoun,
  isValidVerbNounCombination,
...
export class DSKYCommandExecutor {
  ...
  /**
   * Constructs a DSKYCommandExecutor.
   * @param web3Service - The Web3 service instance.
   * @param coinListManager - The coin list manager instance.
   */
  constructor(
  ...
  /**
   * Executes a verb/noun command and returns the result.
   * @param verb - The verb code as a string.
   * @param noun - The noun code as a string.
   * @param currentWeb3State - The current Web3 state.
  ...
  /**
   * Dispatches to the correct handler for the verb/noun combination.
   * @private
   */
  private async executeCommand(
    ...
    switch (verbNum) {
      ...
      default:
        return {
          success: false,
          statusMessage: STATUS_MESSAGES.COMMAND_NOT_IMPLEMENTED(
            verbNum.toString(),
            nounNum.toString(),
          ),
          dskyUpdates: { oprErr: true },
  ...
  /**
   * Handles wallet connection commands.
   * @param nounNum - The noun number indicating the wallet address.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleWalletConnect(
    ...
    if (nounNum === DSKYNoun.NOUN_WALLET_ADDRESS) {
      const connection =
        (await this.web3Service.connect()) as IWalletConnection;
      return {
        success: true,
  ...
  /**
   * Handles wallet disconnection commands.
   * @param nounNum - The noun number indicating the wallet address.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  ...
  /**
   * Handles wallet balance inquiries.
   * @param nounNum - The noun number indicating the wallet balance.
   * @param currentWeb3State - The current Web3 state.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  ...
  /**
   * Handles current block inquiries.
   * @param nounNum - The noun number indicating the current block.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleCurrentBlock(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (nounNum === DSKYNoun.NOUN_CURRENT_BLOCK) {
      const blockInfo = (await this.web3Service.getCurrentBlock()) as {
        number: number;
      };
      return {
        success: true,
  ...
  /**
   * Handles network status inquiries.
   * @param nounNum - The noun number indicating the network name.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleNetworkStatus(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (nounNum === DSKYNoun.NOUN_NETWORK_NAME) {
      const networkInfo =
        (await this.web3Service.getNetworkInfo()) as IBlockchainData;
      return {
        success: true,
        statusMessage: STATUS_MESSAGES.NETWORK_INFO(
  ...
  /**
   * Handles gas price inquiries.
   * @param nounNum - The noun number indicating the gas price.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleGasPrice(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (nounNum === DSKYNoun.NOUN_GAS_PRICE) {
      const gasData = await this.web3Service.getGasPrice();
      const gasInGwei =
        typeof gasData === "object" && gasData && "standard" in gasData
  ...
  /**
   * Handles wallet information requests.
   * @param nounNum - The noun number indicating the wallet address.
   * @param currentWeb3State - The current Web3 state.
   * @returns ICommandExecutionResult indicating the result of the operation.
   * @private
   */
  private handleWalletInfo(
    ...
    if (nounNum === DSKYNoun.NOUN_WALLET_ADDRESS && currentWeb3State.account) {
      return {
        success: true,
        statusMessage: STATUS_MESSAGES.WALLET_MONITORING(
          currentWeb3State.account,
        ),
        dskyUpdates: {
   ...
   * @param nounNum - The noun number indicating the system status.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  ...
  /**
   * Handles cryptocurrency price requests.
   * @param nounNum - The noun number indicating the cryptocurrency.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleCryptoPrice(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    try {
      let symbol: string;
      ...
      return {
        ...
        dskyUpdates: {
          ...
          reg2: priceDisplay,
          reg3: (cryptoData.change24h ?? cryptoData.priceChange24h ?? 0)
            .toFixed(1)
            .slice(0, 5),
        },
      };
  ...
  /**
   * Handles requests for the coin list.
   * @param nounNum - The noun number indicating the coin list.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleGetCoinList(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
  ...
  /**
   * Handles requests for dynamic coin prices.
   * @param nounNum - The noun number indicating the coin.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleGetCoinPrice(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (
      nounNum >= DSKYNoun.NOUN_DYNAMIC_COIN_START &&
      nounNum <= DSKYNoun.NOUN_DYNAMIC_COIN_END
    ) {
      try {
        ...
        this.coinListManager.actions.updateCoinPrice(coin.id, {
          id: coin.id,
          symbol: coin.symbol,
          name: coin.name,
          lastUpdated:
            typeof priceData.lastUpdated === "string" || typeof priceData.lastUpdated === "number"
              ? new Date(priceData.lastUpdated)
              : priceData.lastUpdated instanceof Date
              ? priceData.lastUpdated
              : new Date(),
        });
        const priceStr = priceData.price?.toFixed(2) || "0.00";
        const changeNum = priceData.priceChange24h ?? priceData.change24h ?? 0;
  ...
  /**
   * Handles coin flagging and unflagging.
   * @param nounNum - The noun number indicating the coin.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleFlagCoin(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (
      (nounNum >= DSKYNoun.NOUN_DYNAMIC_COIN_START &&
        nounNum <= DSKYNoun.NOUN_DYNAMIC_COIN_END) ||
      (nounNum >= DSKYNoun.NOUN_CRYPTO_BITCOIN &&
        nounNum <= DSKYNoun.NOUN_CRYPTO_TOP10)
    ) {
      try {
        const coin = this.coinListManager.actions.getCoinByNoun(nounNum);
        if (!coin) {
  ...
  /**
   * Handles batch price requests for flagged coins.
   * @param nounNum - The noun number indicating the batch operation.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleBatchPrices(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (
      nounNum === DSKYNoun.NOUN_FLAGGED_COINS ||
      nounNum === DSKYNoun.NOUN_BATCH_RESULTS
    ) {
      try {
        ...
        if (flaggedCoins.length === 0) {
          return {
            success: false,
            statusMessage:
              "No coins flagged for batch operation. Use V37N[coin] to flag coins.",
            dskyUpdates: { oprErr: true },
          };
        }
        ...
        batchResults.forEach((result: IBatchPriceResult) => {
          if (result.success && result.price > 0 && result.id) {
            this.coinListManager.actions.updateCoinPrice(result.id, {
              id: result.id,
              lastUpdated:
  ...
  /**
   * Clears all coin flags.
   * @param nounNum - The noun number indicating the coin flags.
   * @returns Promise resolving to the command execution result.
   * @private
   */
  private async handleClearFlags(
    nounNum: number,
  ): Promise<ICommandExecutionResult> {
    if (
      nounNum === DSKYNoun.NOUN_COIN_FLAGS ||
      nounNum === DSKYNoun.NOUN_FLAGGED_COINS
    ) {
      try {
        const flaggedCount = this.coinListManager.state.flaggedCoins.size;
  ...
  /**
   * Returns a result for an invalid noun.
   * @param nounNum - The invalid noun number.
   * @returns ICommandExecutionResult indicating error.
   * @private
   */
