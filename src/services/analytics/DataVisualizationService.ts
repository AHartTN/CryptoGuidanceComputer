...
export class DataVisualizationService {
  ...
  generateDashboardData(): {
    ...
    return {
      ...
      blockchainCharts: allViz.filter((v) =>
        v.config.dataSource.includes("blockchain"),
      ),
      analyticsCharts: allViz.filter((v) =>
        v.config.dataSource.includes("analytics"),
      ),
  ...
  private generateSummaryStats(): Record<string, unknown> {
    ...
    return {
      ...
      activeVisualizations: visualizations.filter(
        (v) => !v.isLoading && !v.error,
      ).length,
