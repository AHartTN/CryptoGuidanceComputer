...
export class DSKYCommandExecutor {
  ...
  constructor(
    ...
    this.coinListManager = coinListManager;
  ...
  private async handleGasPrice(
    ...
    if (nounNum === DSKYNoun.NOUN_GAS_PRICE) {
      ...
      const gasInGwei = formatGasPriceToGwei(gasData as import("../../interfaces/IGasPriceData").IGasPriceData);
      return {
        success: true,
  ...
  private handleWalletInfo(
    ...
    if (nounNum === DSKYNoun.NOUN_WALLET_ADDRESS && currentWeb3State.account) {
      const addressRegisters = formatAddressForRegisters(
        currentWeb3State.account,
      );
      return {
        success: true,
  ...
  private async handleCryptoPrice(
    ...
    try {
      ...
      const changeDisplay = formatChangeForRegister(cryptoData.change24h ?? 0);
    ...
    if (
      ...
      try {
        ...
        this.coinListManager.actions.updateCoinPrice(coin.id, {
          ...
          symbol: coin.symbol,
          name: coin.name,
          lastUpdated: (typeof priceData.lastUpdated === "string" || typeof priceData.lastUpdated === "number")
  ...
  private async handleBatchPrices(
    ...
    if (
      ...
      try {
        ...
        const batchResults =
          await this.dynamicCryptoService.getBatchPrices(coinIds);
        ...
        batchResults.forEach((result: IBatchPriceResult) => {
          if (result.success && result.price > 0 && result.id) {
            this.coinListManager.actions.updateCoinPrice(result.id, {
              id: result.id,
              lastUpdated: result.timestamp ? new Date(result.timestamp) : new Date(),
            });
          }
        });

        const successCount = batchResults.filter(
          (r: IBatchPriceResult) => r.success,
        ).length;
