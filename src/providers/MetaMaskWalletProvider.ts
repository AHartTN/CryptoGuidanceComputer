...
export class MetaMaskWalletProvider extends BaseWalletProvider {
  ...
  async connect(): Promise<IWalletConnection> {
    ...
    try {
      ...
      if (!window.ethereum) throw this.createError(WalletErrorType.NotInstalled, 'MetaMask not available');
      ...
      throw this.createError(WalletErrorType.UnknownError, `Connection failed: ${(error as Error).message}`);
    }
  }
  ...
  async requestAccounts(): Promise<string[]> {
    ...
    try {
      ...
      if (!Array.isArray(result) || !result.every(a => typeof a === 'string')) {
        throw this.createError(WalletErrorType.UnknownError, 'Invalid accounts result');
      }
      ...
      throw this.createError(WalletErrorType.UnknownError, `Failed to request accounts: ${(error as Error).message}`);
    }
  }
  ...
  async sendTransaction(request: ITransactionRequest): Promise<string> {
    ...
    try {
      ...
      if (typeof txHash !== 'string') {
        throw this.createError(WalletErrorType.UnknownError, 'Invalid transaction hash');
      }
      return txHash;
    } catch (error: unknown) {
      if ((error as { code?: number }).code === 4001) {
      ...
      throw this.createError(WalletErrorType.UnknownError, `Transaction failed: ${(error as Error).message}`);
    }
  }
  ...
  async signMessage(message: string): Promise<string> {
    ...
    try {
      ...
      if (typeof sig !== 'string') {
        throw this.createError(WalletErrorType.UnknownError, 'Invalid signature');
      }
      return sig;
    } catch (error: unknown) {
      if ((error as { code?: number }).code === 4001) {
      ...
      throw this.createError(WalletErrorType.UnknownError, `Message signing failed: ${(error as Error).message}`);
    }
  }
  ...
  async signTypedData(typedData: Record<string, unknown>): Promise<string> {
    ...
    try {
      ...
      throw this.createError(WalletErrorType.UnknownError, `Typed data signing failed: ${(error as Error).message}`);
    }
  }
  ...
  async switchNetwork(request: INetworkSwitchRequest): Promise<void> {
    ...
    try {
      ...
      if ((error as { code?: number }).code === 4902) {
        ...
        throw this.createError(WalletErrorType.NetworkError, `Network switch failed: ${(error as Error).message}`);
      }
    }
  }
  ...
  async addNetwork(request: INetworkSwitchRequest): Promise<void> {
    ...
    try {
      ...
      throw this.createError(WalletErrorType.NetworkError, `Adding network failed: ${(error as Error).message}`);
    }
  }
  ...
  async getCurrentNetwork(): Promise<{ chainId: string; chainName: string }> {
    ...
    try {
      ...
      throw this.createError(WalletErrorType.NetworkError, `Failed to get current network: ${(error as Error).message}`);
    }
  }
  ...
  onDisconnect(_callback: (error: IWalletError | null) => void): void {
    // Implementation intentionally left blank or handled elsewhere
  }
